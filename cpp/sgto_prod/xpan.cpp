        ...
        for (int m1 = -l1; m1 <= l1; ++m1) {
            ...
            for (int l2 = 0; l2 <= LMAX; ++l2) {
                for (int m2 = -l2; m2 <= l2; ++m2) {
                    ...
                    for (int l1p = 0; l1p <= l1; ++l1p) {
                        int l1pp = l1 - l1p;
                        for (int m1p = std::max(-l1p, m1+l1p-l1);
                                m1p <= std::min(l1p, m1+l1-l1p); ++m1p) {
                            int m1pp = m1 - m1p;
                            int l1pm1p = pack_lm(l1p, m1p);
                            auto mc1 = MC1[l1m1*SZ1+l1pm1p];
                            for (int l2p = 0; l2p <= l2; ++l2p) {
                                int l2pp = l2 - l2p;
                                for (int m2p = std::max(-l2p, m2+l2p-l2);
                                        m2p <= std::min(l2p, m2+l2-l2p); ++m2p) {
                                    int m2pp = m2 - m2p;
                                    int l2pm2p = pack_lm(l2p, m2p);
                                    auto mc2 = MC2[l2m2*SZ1+l2pm2p];
                                    int m = m1pp + m2pp;
                                    ...
                                    for (int q = 0; q <= qmax; ++q) {
                                        ...
                                        coef[ir*SZX+ic] += G[gind(l1pp, m1pp, l2pp, m2pp, q)] * mc1 * mc2;
                                    }
                                }
                            }
                        }
